        ...
        fun readCpi(
            ...
            ByteArrayInputStream(buffer).use {
                return when (formatVersion) {
                    version1 -> CpiLoader.loadCpi(inputStream, expansionLocation, cpiLocation, verifySignature)
                    version2 -> throw UnknownFormatVersionException("Implement format version 2")
                    else -> throw UnknownFormatVersionException("Unknown CPK format version \"$formatVersion\"")
                }
            }
        }
    }
